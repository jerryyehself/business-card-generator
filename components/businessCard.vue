...
function openSocialMedia(site) {
    window.open(`https://${socialMediaSetting[site].url}/${person.socialMedia[site]}`)
}

</script>
<template>
    <div
        class="border rounded-lg p-4 w-[336px] aspect-[5/3] bg-opacity-30 bg-slate-300 flex flex-col text-white text-xs">
        <div class="flex-1 overflow-auto">
            <div class="grid grid-rows-4 gap-2 h-full">
                <div class="row-span-3 grid grid-cols-5">
                    <div class="col-span-2 h-full pr-2 flex flex-col items-center justify-center">
                        <div class="w-20 h-20 flex flex-grow items-center">
                            <img :src="person.photo" class="w-full object-cover rounded-full">
                        </div>
                        <div class="px-4">
                            <p class="font-semibold tracking-wide">{{ person.personAttribute.nick }}</p>
                            <p class="">{{ person.personAttribute.main }}</p>
                        </div>
                    </div>
                    <div class="col-span-3 gap-1 h-full"
                        :class="`grid grid-rows-${businessCard.input.authority.length}`">
                        <div v-for="field in businessCard.input.authority" class="gap-2 grid grid-cols-5">
                            <span class="font-mono font-extrabold font-stretch-extra-expanded col-span-2">
                                {{ field.name }}
                            </span>
                            <span class="col-span-3">{{ person.authority[field.label] }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-center gap-3">
                    <button
                        class="w-5 h-5 flex items-center justify-center relative overflow-hidden rounded-full bg-white group transition-all duration-300"
                        ...
                        <div class="absolute top-full left-0 w-full h-full rounded-full bg-blue-500 z-0
                                transition-all
                                duration-500 group-hover:top-0"></div>
                    </button>
                    <button
                        class="w-5 h-5 flex items-center justify-center rounded-full relative overflow-hidden bg-white group transition-all duration-500"
                        @click.prevent="openSocialMedia('instagram')">
                        <svg class="fill-gray-900 relative z-10 transition-all duration-500 group-hover:fill-white"
                            xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 51 51" fill="none">
                            <path
                        ...
                        <div class="absolute top-full left-0 w-full h-full rounded-full bg-gradient-to-bl
                                from-purple-500 via-pink-500 to-yellow-500 z-0 transition-all duration-500
                                group-hover:top-0"></div>
                    </button>
